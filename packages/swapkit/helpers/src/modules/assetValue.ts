...
You can load static assets by installing @lastnetwork/tokens package and calling AssetValue.loadStaticAssets()
or by passing asyncTokenLookup: true to the from() function, which will make it async and return a promise.`,
  ...
  static loadStaticAssets() {
    return new Promise<{ ok: true } | { ok: false; message: string; error: Todo }>(
      (resolve, reject) => {
        try {
          import("@lastnetwork/tokens").then((tokenPackages) => {
          ...
          reject({
            ...
            error,
            message:
              "Couldn't load static assets. Ensure you have installed @lastnetwork/tokens package",
          });
        }
      },
    );
